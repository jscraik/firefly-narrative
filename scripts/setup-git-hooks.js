...
const REQUIRED_HOOKS = {
	...
	"pre-push": "pnpm check && pnpm test:deep",
};
...
function main() {
	...
	// Enforce simple-git-hooks version
	const deps = packageJson.devDependencies;
	if (deps["simple-git-hooks"] !== "^2.13.1") {
		deps["simple-git-hooks"] = "^2.13.1";
		console.info("✓ Enforced simple-git-hooks version");
		modified = true;
	}
	...
	// Enforce postinstall script
	const scripts = packageJson.scripts;
	if (scripts.postinstall !== "simple-git-hooks") {
		scripts.postinstall = "simple-git-hooks";
		console.info("✓ Enforced postinstall script");
		modified = true;
	}

	// Add/align simple-git-hooks configuration
	if (!packageJson["simple-git-hooks"]) {
		packageJson["simple-git-hooks"] = {};
		console.info("✓ Added simple-git-hooks configuration object");
		modified = true;
	}
	...
	for (const [hookName, hookCommand] of Object.entries(REQUIRED_HOOKS)) {
		if (hooks[hookName] !== hookCommand) {
			...
			console.info(`✓ Set ${hookName} hook to: ${hookCommand}`);
			modified = true;
		}
	}
	...
	console.info("  • pre-push: pnpm check && pnpm test:deep");
}

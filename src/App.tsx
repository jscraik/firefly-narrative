...
import { DASHBOARD_FOCUS_RESTORE_MS } from "./ui/views/dashboardState";
...
export function App() {
	...
	const [surfaceAction, setSurfaceAction] = useState<
		import("./ui/views/narrativeSurfaceData").SurfaceAction | undefined
	>(undefined);
	...
	const [AgentationComponent, setAgentationComponent] =
		useState<AgentationComponentType | null>(null);
	const rawAgentationEndpoint = import.meta.env.VITE_AGENTATION_ENDPOINT as
		| string
		| undefined;
	const normalizedAgentationEndpoint = normalizeHttpUrl(rawAgentationEndpoint);
	const isAgentationEnabled =
		import.meta.env.DEV && Boolean(normalizedAgentationEndpoint);

	const rawAgentationWebhookUrl = import.meta.env.VITE_AGENTATION_WEBHOOK_URL as
		| string
		| undefined;
	const normalizedAgentationWebhookUrl = normalizeHttpUrl(
		rawAgentationWebhookUrl,
	);
	const agentationWebhookUrl =
		normalizedAgentationWebhookUrl ?? "http://localhost:8787";

	useEffect(() => {
		if (typeof window === "undefined") return;
		const stored = window.localStorage.getItem(
			"narrative.githubConnector.enabled",
		);
		if (stored === "true") {
	...
	useEffect(() => {
		...
		// rawAgentationWebhookUrl set but invalid — URL normalization handles the warning
	}, [isAgentationEnabled, normalizedAgentationEndpoint]);
	...
	const modelForHooks =
		repoState.status === "ready" ? repoState.model : EMPTY_BRANCH_MODEL;
	...
	const commitData = useCommitData({
		mode,
		repoState,
		diffCache: diffCache as unknown as MutableRefObject<{

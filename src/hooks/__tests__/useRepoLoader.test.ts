...
function createModel(repoId: number): BranchViewModel {
  return {
    ...
    intent: [],
    timeline: [{ id: `sha-${repoId}`, type: 'commit', label: `Commit ${repoId}` }],
    meta: {
      repoId,
...
describe('useRepoLoader', () => {
  beforeEach(() => {
    ...
    mockDetectCodexOtelPromptExport.mockResolvedValue({ enabled: null, configPath: null });
  });
  ...
  it('ignores stale attribution preference responses when repo changes', async () => {
    ...
    mockGetAttributionPrefs.mockImplementation((repoId: number) => deferredByRepo[repoId].promise);
  ...
  it('ignores stale openRepo success completion when a newer repo load finishes first', async () => {
    const deferredByPath: Record<string, Deferred<ReturnType<typeof createIndexResult>>> = {
    ...
    mockIndexRepo.mockImplementation(async (selectedPath: string) => deferredByPath[selectedPath].promise);
    ...
    await waitFor(() => {
      expect(mockIndexRepo).toHaveBeenCalledWith('/repo/2', 60, expect.any(Function));
    });
  ...
  it('ignores stale preference update result after repo switch', async () => {
    ...
    await act(async () => {
      updateRequest = result.current.updateAttributionPrefs({ showLineOverlays: false });
      await Promise.resolve();
    });

    await waitFor(() => {
      expect(mockSetAttributionPrefs).toHaveBeenCalledWith(1, { showLineOverlays: false });
    });
  ...
  it('ignores stale purge refresh result after repo switch', async () => {
    ...
    mockGetAttributionPrefs.mockImplementation(async (repoId: number) => createPrefs(repoId));
    mockPurgeAttributionPromptMeta.mockImplementation(async (repoId: number) => {

...
import { autoCaptureIfNeeded, listSnapshots } from "../core/repo/snapshots";
...
export function useSnapshots(params: {
	...
	const refreshSnapshots = useCallback(async () => {
		...
		try {
			...
			const { dirtyFiles, dirtyChurnLines } =
				await autoCaptureIfNeeded(requestRepoRoot);
			if (
				!isMountedRef.current ||
				latestRepoRootRef.current !== requestRepoRoot
			)
				return;
			...
			setRepoState((prev) => {
				...
				if (prev.meta?.repoPath && prev.meta.repoPath !== requestRepoRoot)
					return prev;

				const oldSnapshots = prev.snapshots || [];
				const oldDirtyFiles = [...(prev.dirtyFiles ?? [])].sort();
				const nextDirtyFiles = [...dirtyFiles].sort();
				const dirtyChurnUnchanged =
					(prev.dirtyChurnLines ?? 0) === dirtyChurnLines;
				const dirtyFilesUnchanged =
					oldDirtyFiles.length === nextDirtyFiles.length &&
					oldDirtyFiles.every((file, index) => file === nextDirtyFiles[index]);

				if (
					dirtyFilesUnchanged &&
					dirtyChurnUnchanged &&
					oldSnapshots.length === snapshots.length
				) {
			...
			console.warn("[useSnapshots] Failed to refresh snapshots:", err);
		}

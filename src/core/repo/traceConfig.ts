...
export async function loadTraceConfig(repoRoot: string): Promise<TraceCollectorConfig> {
  try {
    const raw = await readNarrativeFile(repoRoot, TRACE_CONFIG_PATH);
    const parsed = JSON.parse(raw) as Partial<TraceCollectorConfig>;
    const path = typeof parsed.codexOtelLogPath === 'string' ? parsed.codexOtelLogPath : DEFAULT_CODEX_OTEL_LOG_PATH;
    const enabled =
...
export async function saveTraceConfig(repoRoot: string, config: TraceCollectorConfig): Promise<void> {
  await writeNarrativeFile(repoRoot, TRACE_CONFIG_PATH, JSON.stringify(config, null, 2));
}

...
async function loadHighlightAdjustments(
	...
	for (const row of rows) {
		...
		adjustments[row.target_id] = round(
			clamp((keyWeight - wrongWeight) * 0.03, -0.15, 0.15),
		);
	}
...
async function recomputeCalibrationProfile(
	repoId: number,
): Promise<NarrativeCalibrationProfile> {
	const db = await getDb();
	...
	const rankingBias = round(
		clamp(
			((keyCount - wrongCount) / Math.max(1, keyCount + wrongCount)) * 0.12,
			-0.15,
			0.15,
		),
	);
	const confidenceOffset = round(
		clamp(
			((keyCount - wrongCount) / effectiveTotal) * 0.1 -
				Math.min(0.08, missingCount * 0.01),
			-0.12,
			0.12,
		),
	);
	...
	const highlightAdjustments = await loadHighlightAdjustments(
		repoId,
		windowStartISO,
	);
...
export async function submitNarrativeFeedback(
	...
	if (
		(input.action.feedbackType === "highlight_key" ||
			input.action.feedbackType === "highlight_wrong") &&
	...
	if (
		input.action.feedbackType === "branch_missing_decision" &&
		input.action.targetKind !== "branch"
	) {
...
export async function getNarrativeCalibrationProfile(
	...
	return {
		...
		actorWeightPolicyVersion:
			row.actor_weight_policy_version ?? CALIBRATION_POLICY_VERSION,

...
export function parseJUnitXml(xmlText: string): ParsedJUnit {
  ...
  if (upper.includes('<!DOCTYPE') || upper.includes('<!ENTITY')) {
    throw new Error('Unsupported JUnit XML (DOCTYPE/ENTITY is not allowed). Please re-export without a DOCTYPE.');
  }
  ...
  if (parseError) {
    throw new Error('Failed to parse JUnit XML. Please confirm the file is valid XML.');
  }
  ...
  for (const tc of Array.from(doc.querySelectorAll('testcase'))) {
    const name = getAttr(tc, 'name') ?? getAttr(tc, 'classname') ?? '(unnamed test)';
    ...
    const status: TestCase['status'] = skippedEl ? 'skipped' : failure || error ? 'failed' : 'passed';

    if (status === 'passed') passed += 1;
    if (status === 'failed') failed += 1;
    if (status === 'skipped') skipped += 1;

    const errorMessage = textContent(failure) ?? textContent(error) ?? undefined;

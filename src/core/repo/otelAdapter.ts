import type { TraceRecord, TraceContributorType, TraceFile } from '../types';
...
const CONVERSATION_KEYS = ['conversation_id', 'codex.conversation_id', 'conversation.id'];
...
function extractValue(value?: OTelValue): string[] {
  ...
  if ('intValue' in value && value.intValue !== undefined) return [String(value.intValue)];
  if ('boolValue' in value && value.boolValue !== undefined) return [String(value.boolValue)];
  if ('doubleValue' in value && value.doubleValue !== undefined) return [String(value.doubleValue)];
...
function attributesToMap(attrs: Array<{ key: string; value: OTelValue }> | undefined): Record<string, string[]> {
...
function mergeAttributes(base: Record<string, string[]>, overlay: Record<string, string[]>): Record<string, string[]> {
  const out = { ...base };
...
function pickFirst(attributes: Record<string, string[]>, keys: string[]): string | null {
...
function pickAll(attributes: Record<string, string[]>, keys: string[]): string[] {
  const out: string[] = [];
  for (const key of keys) {
    const values = attributes[key];
...
function buildFallbackTraceFiles(fileHints: string[], modelId: string | null): TraceFile[] {
  ...
  return [
    {
      ...
      conversations: [
        {
          url: undefined,
          contributor: { type: toContributorType(modelId), modelId: modelId ?? undefined },
          ranges: [
            {
              startLine: 1,
              endLine: 1,
...
export async function codexOtelEventsToTraceRecords(options: {
  ...
  for (const event of events) {
    ...
    grouped[commitSha] = grouped[commitSha] ? [...grouped[commitSha], event] : [event];
  }
  ...
  for (const [commitSha, commitEvents] of Object.entries(grouped)) {
    ...
    const conversationId = pickFirst(commitEvents[0]?.attributes ?? {}, CONVERSATION_KEYS);
    const fileHints = pickAll(commitEvents[0]?.attributes ?? {}, FILE_KEYS);

    let traceFiles: TraceFile[] = [];
    try {
      traceFiles = await buildTraceFilesFromCommit(repoRoot, commitSha, fileHints, modelId);
    } catch {
      errors.push({ commitSha, message: 'Failed to build trace ranges from git diff' });
    }

    if (traceFiles.length === 0) {
      errors.push({ commitSha, message: 'No diff ranges found for commit; using fallback' });
    ...
    records.push({
      ...
      tool: { name: 'codex', version: pickFirst(commitEvents[0].attributes, ['app.version', 'codex.version']) ?? undefined },
      files: traceFiles,
      metadata: {
        'dev.narrative': {
          derived: true,

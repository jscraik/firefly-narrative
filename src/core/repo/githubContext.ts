...
function sanitizeUntrustedText(text: string | undefined): { value?: string; redactionHits: number } {
  ...
  const escaped = withoutControls.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
  const redacted = redactSecrets(escaped);
  const redactionHits = redacted.hits.reduce((total, hit) => total + hit.count, 0);
...
function toEntry(path: string, raw: unknown): GitHubContextEntry | null {
  ...
  const pr = (record.pull_request ?? record.pr) as Record<string, unknown> | undefined;
  ...
  return {
    ...
    number,
    updatedAtISO,
    redactionHits: title.redactionHits + body.redactionHits + reviewSummary.redactionHits,
  };
}

export async function loadGitHubContext(repoRoot: string): Promise<GitHubContextState> {
  try {
    const files = await listNarrativeFiles(repoRoot, 'connectors/github');
    const jsonFiles = files.filter((file) => file.toLowerCase().endsWith('.json')).sort((a, b) => b.localeCompare(a));

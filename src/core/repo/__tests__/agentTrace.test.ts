...
describe("parseTraceRecord (via importAgentTraceFile)", () => {
	...
	it("throws on invalid JSON input", async () => {
		...
		vi.mocked(redactSecrets).mockReturnValue({
			redacted: "not-json-at-all",
			hits: [],
		});

		await expect(
			importAgentTraceFile("/repo", 1, "/path/bad.json"),
		).rejects.toThrow("Invalid Agent Trace record");
	});

	it("throws on JSON that is not an object", async () => {
		vi.mocked(readTextFile).mockResolvedValue('"just a string"');
		vi.mocked(redactSecrets).mockReturnValue({
			redacted: '"just a string"',
			hits: [],
		});

		await expect(
			importAgentTraceFile("/repo", 1, "/path/scalar.json"),
		).rejects.toThrow("Invalid Agent Trace record");
	});
	...
	it("throws when required fields (id) are missing", async () => {
		...
		await expect(
			importAgentTraceFile("/repo", 1, "/path/missing-id.json"),
		).rejects.toThrow("Invalid Agent Trace record");
	});
	...
	it("throws when required fields (vcs) are missing", async () => {
		...
		await expect(
			importAgentTraceFile("/repo", 1, "/path/missing-vcs.json"),
		).rejects.toThrow("Invalid Agent Trace record");
	});
	...
	it('throws when vcs.type is not "git"', async () => {
		...
		await expect(
			importAgentTraceFile("/repo", 1, "/path/svn.json"),
		).rejects.toThrow("Invalid Agent Trace record");
	});
	...
	it("throws when files array is empty", async () => {
		...
		await expect(
			importAgentTraceFile("/repo", 1, "/path/empty-files.json"),
		).rejects.toThrow("Invalid Agent Trace record");
	});
	...
	it("returns redaction hits from redactSecrets", async () => {
		...
		const result = await importAgentTraceFile(
			"/repo",
			1,
			"/path/has-secrets.json",
		);
...
describe("ingestTraceRecord", () => {
	...
	it("inserts trace_records, trace_files, and trace_conversations for a new record", async () => {
		...
		expect(
			sqls.some((s) => s.includes("INSERT OR IGNORE INTO trace_records")),
		).toBe(true);
		expect(sqls.some((s) => s.includes("INSERT INTO trace_files"))).toBe(true);
		expect(
			sqls.some((s) => s.includes("INSERT INTO trace_conversations")),
		).toBe(true);
...
describe("ingestCodexOtelLogFile", () => {
	...
	it("returns inactive status when OTel log file is not found", async () => {
		...
		const result = await ingestCodexOtelLogFile({
			repoRoot: "/repo",
			repoId: 1,
		});
	...
	it("returns error status for unexpected read errors", async () => {
		...
		expect((result.status as { message?: string }).message).toBe(
			"permission denied",
		);
	});
	...
	it("downgrades commit fallback notices to informational (active, not partial)", async () => {
		...
		expect((result.status as { message?: string }).message).toContain(
			fallbackMsg,
		);
	...
	it("uses custom logPath when provided", async () => {
		...
		expect(vi.mocked(readTextFile)).toHaveBeenCalledWith(
			"/custom/path/otel.json",
		);
	});
...
describe("generateDerivedTraceRecord", () => {
	...
	it("builds a valid TraceRecord from git diff ranges", async () => {
		...
		expect(record.files[0].conversations[0].ranges[0]).toEqual({
			startLine: 1,
			endLine: 10,
		});
	...
	it("sets sessionId to null in metadata when not provided", async () => {
		...
		expect(
			(record.metadata?.["dev.narrative"] as { sessionId?: unknown }).sessionId,
		).toBeNull();
	});
	...
	it("assigns a new unique id per call", async () => {
		...
		const r1 = await generateDerivedTraceRecord({
			repoRoot: "/repo",
			commitSha: "abc",
			files,
		});
		const r2 = await generateDerivedTraceRecord({
			repoRoot: "/repo",
			commitSha: "abc",
			files,
		});

...
function buildFixture() {
  return fixtureRaw.replace("__COMMIT_SHA__", commitSha).replace("__FILE_PATH__", filePath);
}
...
describe("otelAdapter", () => {
  ...
  it("reports missing commit SHA", async () => {
    ...
    expect(conversion.errors.some((error) => /missing commit sha/i.test(error.message))).toBe(true);
  });
});

...
import { getOrLoadCommitFiles, indexRepo, type RepoIndex } from "../indexer";
...
const mockGetLatestTestRunSummaryByCommit = vi.mocked(
...
describe("indexer", () => {
	const mockCommits = [
		{
			...
			author: "Test Author",
			authoredAtISO: "2024-01-01T00:00:00Z",
		},
		{
			sha: "def456",
			subject: "Second commit",
			author: "Test Author",
			authoredAtISO: "2024-01-02T00:00:00Z",
		},
	];
	...
	beforeEach(() => {
		...
		mockImportAttributionNotesBatch.mockResolvedValue({
			total: 0,
			imported: 0,
			missing: 0,
			failed: 0,
	...
	describe("indexRepo", () => {
		...
		it("should handle test run hydration failure gracefully", async () => {
			mockGetLatestTestRunSummaryByCommit.mockRejectedValue(
	...
	describe("getOrLoadCommitFiles", () => {
		...
		it("should fetch from git when not cached", async () => {
			...
			expect(cacheFileChanges).toHaveBeenCalledWith(1, {
				sha: "abc123",
				fileChanges: mockFileChanges,
			});
			expect(result).toEqual(mockFileChanges);
		...
		it("should write metadata after fetching from git", async () => {
			...
			expect(writeCommitFilesMeta).toHaveBeenCalledWith(
				"/test/repo",
				"abc123",
				mockFileChanges,
			);
		...
		it("should handle metadata write failure gracefully", async () => {
			...
			vi.mocked(cacheFileChanges).mockResolvedValue(undefined);
			vi.mocked(writeCommitFilesMeta).mockRejectedValue(

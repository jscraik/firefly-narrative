...
export async function writeRepoMeta(repoRoot: string, payload: unknown) {
  await writeNarrativeFile(repoRoot, 'meta/repo.json', JSON.stringify(payload, null, 2));
}

export async function writeBranchMeta(repoRoot: string, branch: string, payload: unknown) {
  const file = `meta/branches/${safeName(branch)}.json`;
  await writeNarrativeFile(repoRoot, file, JSON.stringify(payload, null, 2));
}

export async function writeCommitSummaryMeta(repoRoot: string, commit: CommitSummary) {
...
export async function writeCommitFilesMeta(repoRoot: string, sha: string, fileChanges: FileChange[]) {
  ...
  await writeNarrativeFile(
    ...
    JSON.stringify(
      {
        sha,
        totals: { added: totalAdded, removed: totalRemoved, files: fileChanges.length },
        files: fileChanges
      },
      null,
      2

...
export async function readAgentNote(
  ...
  try {
    const stdout = await git(repoRoot, ['notes', `--namespace=${NAMESPACE}`, 'show', commitSha]);
...
export async function writeAgentNote(
  ...
  await git(repoRoot, ['notes', `--namespace=${NAMESPACE}`, 'add', '-f', '-m', noteJson, commitSha]);
}
...
export async function removeAgentNote(
  ...
  try {
    await git(repoRoot, ['notes', `--namespace=${NAMESPACE}`, 'remove', commitSha]);
  } catch {
...
export async function listAgentNotes(
  ...
  try {
    const stdout = await git(repoRoot, ['notes', `--namespace=${NAMESPACE}`, 'list']);

...
function inferTradeoffs(reason: string): string[] {
  ...
  if (lower.includes('test')) {
    return ['Delivery speed vs reliability hardening', 'Coverage expansion vs merge velocity'];
  }
  if (lower.includes('attribution') || lower.includes('trace')) {
    return ['Transparency depth vs interface complexity', 'Evidence richness vs runtime cost'];
  }
  return ['Narrative clarity vs implementation scope', 'Confidence calibration vs generation speed'];
}
...
function inferAlternatives(reason: string): string[] {
  ...
  return ['Implement as a direct diff-only workflow', 'Defer deeper context until later phase'];
}
...
export function buildDecisionArchaeology(args: {
  ...
  if (githubEntry?.reviewSummary) {
    entries.push({
      ...
      tradeoffs: ['External context depth vs ingestion complexity', 'Review breadth vs redaction strictness'],

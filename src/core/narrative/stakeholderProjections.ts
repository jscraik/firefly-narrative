...
function topRisk(narrative: BranchNarrative): string {
  if (narrative.state === 'needs_attention') {
    return narrative.fallbackReason ?? 'Narrative confidence is low; verify via evidence.';
  }
...
export function buildStakeholderProjections(args: {
  ...
  return {
    ...
    manager: {
      ...
      bullets: [
        ...highlights.map((highlight) => `${highlight.title}: ${highlight.whyThisMatters}`),
        githubEntry?.reviewSummary
          ? `Review context: ${githubEntry.reviewSummary}`
          : 'No external review context linked yet.',
      ],
      risks: [risk, 'Track manual pin/unpin feedback to improve highlight precision.'],
      evidenceLinks: sharedEvidence,
    },
    engineer: {
      audience: 'engineer',

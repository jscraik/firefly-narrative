...
export function compareRecallLaneItems(
  ...
  const tierDelta = TIER_WEIGHT[b.item.confidenceTier] - TIER_WEIGHT[a.item.confidenceTier];
  if (tierDelta !== 0) return tierDelta;

  const sourceDelta = SOURCE_WEIGHT[b.item.source] - SOURCE_WEIGHT[a.item.source];
...
function normalizeEvidenceLinks(links: unknown): NarrativeEvidenceLink[] {
  ...
  return links
    .filter((link): link is NarrativeEvidenceLink => {
      ...
      return typeof candidate.id === 'string' && typeof candidate.kind === 'string' && typeof candidate.label === 'string';
    })
    .map((link) => ({
      ...link,
      id: String(link.id),
...
function buildFallbackFromNarrative(narrative: BranchNarrative): NarrativeRecallLaneItem {

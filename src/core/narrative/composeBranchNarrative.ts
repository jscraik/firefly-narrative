...
function reasonForNode(node: TimelineNode): string {
  ...
  const hasFailedTests = badges.some((badge) => badge.type === 'test' && badge.status === 'failed');
  if (hasFailedTests) {
...
function uniqueEvidence(links: NarrativeEvidenceLink[]): NarrativeEvidenceLink[] {
...
function buildHighlights(model: BranchViewModel, commitNodes: TimelineNode[]): NarrativeHighlight[] {
  const highlights: NarrativeHighlight[] = [];
  const recentCommits = [...commitNodes].reverse().slice(0, 3);

  for (const node of recentCommits) {
    const badges = node.badges ?? [];
    const hasAnchor = badges.some((badge) => badge.type === 'anchor' && badge.status !== 'failed');
  ...
  if (highlights.length === 0 && model.intent.length > 0) {
    ...
    highlights.push({
      ...
      title: item.text,
      whyThisMatters: 'Intent extracted from commit/session context provides the best available narrative starting point.',
      confidence: 0.42,
      evidenceLinks: [],
    });
  }
...
export function composeBranchNarrative(model: BranchViewModel): BranchNarrative {
  ...
  const hasSessions = Boolean(model.sessionExcerpts && model.sessionExcerpts.length > 0);

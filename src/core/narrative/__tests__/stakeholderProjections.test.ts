...
function model(): BranchViewModel {
  return {
    ...
    stats: { added: 10, removed: 4, files: 2, commits: 1, prompts: 1, responses: 2 },
  ...
  highlights: [
    {
      ...
      confidence: 0.68,
      evidenceLinks: [{ id: 'commit:abc123', kind: 'commit', label: 'Commit abc123', commitSha: 'abc123' }],
    },
  ],
  evidenceLinks: [{ id: 'commit:abc123', kind: 'commit', label: 'Commit abc123', commitSha: 'abc123' }],
};
...
describe('buildStakeholderProjections', () => {
  it('returns all audience projections with headline and bullets', () => {
    const projections = buildStakeholderProjections({ narrative, model: model() });

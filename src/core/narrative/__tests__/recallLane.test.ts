...
import type { BranchNarrative, NarrativeConfidenceTier, NarrativeRecallLaneItemSource, NarrativeEvidenceLink } from '../../types';
...
describe('compareRecallLaneItems', () => {
  ...
  it('orders by source when tier is equal (highlight > fallback)', () => {
    ...
    expect(compareRecallLaneItems(fallbackSource, highlightSource)).toBeGreaterThan(0);
  });
...
describe('buildRecallLane', () => {
  ...
  it('filters items below confidenceFloor', () => {
    const lane = buildRecallLane(narrative, { maxItems: 3, confidenceFloor: 0.7 });
  ...
  it('falls back when all highlights filtered by confidenceFloor', () => {
    const lane = buildRecallLane(
      {
        ...narrative,
        highlights: narrative.highlights[0] ? [{ ...narrative.highlights[0], confidence: 0.1 }] : [], // Low confidence
      },
        ...
        highlights: [
          {
            ...
            evidenceLinks: [
              ...
              { id: 'invalid-no-kind', label: 'No Kind' } as unknown as NarrativeEvidenceLink,
              { kind: 'commit', label: 'No ID' } as unknown as NarrativeEvidenceLink,
  ...
  it('returns empty array on exception in buildRecallLane', () => {
    ...
    const lane = buildRecallLane(throwingNarrative as unknown as BranchNarrative, { maxItems: 3 });

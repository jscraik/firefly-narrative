...
export type AtlasErrorCode = (typeof KNOWN_ATLAS_ERROR_CODES)[number] | (string & {});
...
function coerceAtlasError(raw: unknown): AtlasError {
  ...
  const code = typeof raw.code === 'string' ? (raw.code as AtlasErrorCode) : 'INVALID_RESPONSE';
  const message = typeof raw.message === 'string' ? raw.message : 'Unknown Atlas error';
  return { code, message };
}

function parseAtlasEnvelope<T>(raw: unknown): AtlasEnvelope<T> {
  if (!isRecord(raw)) {
    return { ok: false, error: { code: 'INVALID_RESPONSE', message: 'Atlas response was not an object' } };
  }

  if (raw.ok === true) {
    const meta = isRecord(raw.meta) ? (raw.meta as AtlasEnvelopeMeta) : undefined;
  ...
  return { ok: false, error: { code: 'INVALID_RESPONSE', message: 'Atlas response missing ok discriminator' } };
}

async function invokeAtlas<T>(command: string, args: Record<string, unknown>): Promise<AtlasEnvelope<T>> {
...
export async function atlasCapabilities(): Promise<AtlasEnvelope<AtlasCapabilities>> {
  return invokeAtlas<AtlasCapabilities>('atlas_capabilities', {});
}

export async function atlasIntrospect(repoId: number): Promise<AtlasEnvelope<AtlasIntrospect>> {
  return invokeAtlas<AtlasIntrospect>('atlas_introspect', { repoId });
}

export async function atlasSearch(request: AtlasSearchRequest): Promise<AtlasEnvelope<AtlasSearchResponse>> {
  return invokeAtlas<AtlasSearchResponse>('atlas_search', { request });
}

export async function atlasGetSession(request: AtlasGetSessionRequest): Promise<AtlasEnvelope<AtlasGetSessionResponse>> {
  return invokeAtlas<AtlasGetSessionResponse>('atlas_get_session', { request });
}

export async function atlasDoctorReport(repoId: number): Promise<AtlasEnvelope<AtlasDoctorReport>> {
  return invokeAtlas<AtlasDoctorReport>('atlas_doctor_report', { repoId });
}

export async function atlasDoctorRebuildDerived(repoId: number): Promise<AtlasEnvelope<AtlasDoctorRebuildSummary>> {
  return invokeAtlas<AtlasDoctorRebuildSummary>('atlas_doctor_rebuild_derived', { request: { repoId } });
}
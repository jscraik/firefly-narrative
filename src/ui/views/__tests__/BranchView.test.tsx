...
const mockFileSelectionState = vi.hoisted(() => ({
	selectedFile: "preselected.ts" as string | null,
}));
...
const mockCreateTelemetryBranchScope = vi.hoisted(() =>
	vi.fn(
		(_repoId?: number | null, branch?: string) =>
			`scope:${branch ?? "unknown"}`,
	),
);
...
let capturedOpenEvidenceHandler:
	...
	| null = null;
let capturedOpenRawDiffHandler:
	| ((laneContext?: {
			source?: "recall_lane";
...
let capturedOpenAskWhyCitationHandler:
	| ((citation: { id: string; type: string; label: string }) => void)
	| null = null;
const mockLoadGitHubContext = vi.hoisted(() =>
	vi.fn().mockResolvedValue({ status: "empty", entries: [] }),
);
const mockGetNarrativeCalibrationProfile = vi.hoisted(() =>
	vi.fn().mockResolvedValue(null),
);
...
describe("BranchView transition and integration coverage", () => {
	...
	it("ignores stale files loader completion after rapid node change", async () => {
		...
		const loadFilesForNode = vi.fn(
			(nodeId: string) => deferredByNode[nodeId].promise,
		);
		...
		await act(async () => {
			deferredByNode.bbbb2222.resolve([
				{ path: "src/new-file.ts", additions: 4, deletions: 1 },
			]);
		...
		await act(async () => {
			deferredByNode.aaaa1111.resolve([
				{ path: "src/stale-file.ts", additions: 1, deletions: 0 },
			]);
			await Promise.resolve();
		});
		...
		expect(screen.getByTestId("files-changed")).not.toHaveTextContent(
			"src/stale-file.ts",
		);
	});
	...
	it("keeps latest request result for ABA transitions", async () => {
		...
		await act(async () => {
			deferredByNode.aaaa1111[1].resolve([
				{ path: "src/final-a.ts", additions: 8, deletions: 2 },
			]);
			await Promise.resolve();
		});
		...
		await act(async () => {
			deferredByNode.bbbb2222[0].resolve([
				{ path: "src/stale-b.ts", additions: 1, deletions: 1 },
			]);
			deferredByNode.aaaa1111[0].resolve([
				{ path: "src/stale-a.ts", additions: 1, deletions: 1 },
			]);
			await Promise.resolve();
		});
	...
	it("ignores stale evidence/open-raw-diff callbacks after branch scope changes", async () => {
		...
		const preCurrentEvidenceOpenedCalls =
			mockTrackNarrativeEvent.mock.calls.filter(
		...
		await waitFor(() => {
			...
			).toBe(preCurrentEvidenceOpenedCalls + 1);
		});
	});
	...
	it("ignores stale ask-why handlers after branch scope changes", async () => {
		...
		await waitFor(() => {
			expect(mockTrackAskWhySubmitted).toHaveBeenCalledTimes(
				beforeStaleCalls + 1,
			);
		});
	...
	it("tracks recall-lane fallback context for raw diff opens", async () => {
		...
		const beforeFallbackCalls = mockTrackNarrativeEvent.mock.calls.filter(
			(call) => call[0] === "fallback_used",
		).length;
	...
	it("emits first_win_completed once for a fallback completion path", async () => {
		...
		await waitFor(() => {
			...
			const completionPayload = completionEvents.at(-1)?.[1] as
	...
	it("resets diff loading state when selected file context is cleared", async () => {
		...
		await waitFor(() => {
			expect(loadDiffForFile).toHaveBeenCalledWith(
				"aaaa1111",
				"preselected.ts",
			);
		});
	...
	it("clears files when commit selection becomes unavailable", async () => {
		const loadFilesForNode = vi.fn(async () => [
			{ path: "src/old-file.ts", additions: 2, deletions: 1 },
		]);
		const props = buildProps({ loadFilesForNode });
		const { rerender } = render(<BranchView {...props} />);
		...
		rerender(
			<BranchView
				{...buildProps({ model: modelWithoutTimeline, loadFilesForNode })}
			/>,
		);
	...
	it("ignores stale repo test-run completion after rapid commit change", async () => {
		...
		mockGetLatestTestRunForCommit.mockImplementation(
			async (_repoId: number, commitSha: string): Promise<TestRun | null> =>
				deferredByCommit[commitSha].promise,
		);
	...
	it("sets github context to error when connector context loading fails", async () => {
		mockLoadGitHubContext.mockRejectedValueOnce(
	...
	it("does not throw when an in-flight loader resolves after unmount", async () => {
		...
		expect(consoleError).not.toHaveBeenCalledWith(
			expect.stringMatching(
				/unmounted component|state update on an unmounted component/i,
			),
		);
	...
	it("emits commit-scoped what_ready telemetry for selected commits", async () => {
		...
		await waitFor(() => {
			const whatReadyEvents = mockTrackNarrativeEvent.mock.calls.filter(
	...
	it("updates telemetry runtime consent config when ingest consent changes", async () => {
		...
		await waitFor(() => {
			expect(mockSetNarrativeTelemetryRuntimeConfig).toHaveBeenCalledWith({
				consentGranted: true,
			});
		});
	...
	it("pulses each newly session-linked commit in order", async () => {
		...
		try {
			const pendingFileLoad = new Promise<FileChange[]>(() => {
				/* intentionally never resolves — simulates pending load */
			});

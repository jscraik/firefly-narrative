...
function SessionRow({
	...
	branchName: string;
	onAction?: (action: Exclude<SurfaceAction, string>) => void;
}) {
	const fileCount =
		session.messages.reduce((acc, m) => acc + (m.files?.length || 0), 0) || 1;
			...
			<div className="flex items-center gap-3 overflow-hidden">
				...
				<div className="flex flex-col truncate">
					<div className="flex items-center gap-2">
						<span className="truncate text-sm font-medium text-text-primary group-hover:text-accent-violet transition-colors">
							{title}
						</span>
			...
			: ([
					{
						id: "mock-1",
						tool: "codex",
						agentName: "Lede card redesign",
						messages: [{ role: "user", text: "" }],
						importedAtISO: new Date(Date.now() - 1000 * 60 * 30).toISOString(),
					},
					{
						id: "mock-2",
						tool: "claude-code",
						agentName: "Fix layout bugs",
						messages: [
							{ role: "user", text: "" },
							{ role: "user", text: "" },
						],
						importedAtISO: new Date(
							...
							<div className="flex flex-col gap-6 mt-2">
								{today.length > 0 && (
									<div className="flex flex-col gap-2">
										<h3 className="text-xs font-semibold uppercase tracking-widest text-text-muted pl-3">
											Today
										</h3>

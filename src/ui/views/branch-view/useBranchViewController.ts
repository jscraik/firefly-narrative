 ...
 */
function deriveTrustStateFromCaptureStatus(
	status: CaptureReliabilityStatus | null | undefined,
): TrustState {
	...
	if (
		status.mode === "DEGRADED_STREAMING" ||
		(status.streamExpected && !status.streamHealthy)
	) {
		return "trust_paused";
	}
...
export function useBranchViewController(
	props: BranchViewProps,
): ComponentProps<typeof BranchViewLayout> {
	const {
		model,
		onModeChange,
		dashboardFilter,
		...
		branchHeaderParityEnabled = (import.meta.env.VITE_BRANCH_HEADER_PARITY_V1 ??
			"true") !== "false",
	} = props;
	...
	const reportTraceSignalError = useCallback(
		(message: string) => {
	...
	const traceSignal = useTraceSignal({
		...
		trackingSettled:
			selectedNodeId !== null && trackingSettledNodeId === selectedNodeId,
		loadingFiles,
		loadingDiff,
		loadingTrace,
		traceRequestedForSelection,
		traceSummary: selectedCommitSha
			? model.traceSummaries?.byCommit[selectedCommitSha]
			: undefined,
	...
	const handleFileClickFromSession = useCallback(
		(path: string) => {
	...
	const handleOpenRawDiff = useCallback(
		(laneContext?: {
			...
			trackNarrativeEvent("fallback_used", {
				...
				confidence: narrative.confidence,
				source:
					laneContext?.source === "recall_lane" ? "recall_lane" : model.source,
				recallLaneItemId: laneContext?.recallLaneItemId,
				recallLaneConfidenceBand: laneContext?.recallLaneConfidenceBand,
				viewInstanceId: narrativeViewInstanceIdRef.current ?? undefined,
				itemId:
					laneContext?.fallbackItemId ??
					laneContext?.recallLaneItemId ??
					activeCommitSha,
				funnelStep: "evidence_ready",
	...
	const handleTraceSignalTrackingSettled = useCallback(
		(payload: TraceSignalTrackingSettlePayload) => {
			if (!selectedNodeId) return;
	...
	const { askWhyState, handleSubmitAskWhy, handleOpenAskWhyCitation } =
		useBranchAskWhyState({
			attemptId: firstWinAttemptId,
			branchScopeKey,
			branchScope: telemetryBranchScope,
			branchName: model.meta?.branchName,
	...
	const rightPanelProps: ComponentProps<typeof RightPanelTabs> = {
		...
		indexedCommitShas: model.timeline

...
function formatLinkConfidence(confidence: number) {
  if (confidence >= 0.8) return { label: 'High Confidence', color: 'text-accent-green bg-accent-green/10 border-accent-green/20' };
  if (confidence >= 0.5) return { label: 'Medium Confidence', color: 'text-accent-amber bg-accent-amber-bg border-accent-amber-light' };
  return { label: 'Low Confidence', color: 'text-accent-red bg-accent-red-bg border-accent-red-light' };
}

// Scaffold Data
const SCAFFOLD_SESSIONS: (SessionExcerpt & { linkConfidence: number })[] = [
  { id: '1', tool: 'Claude Code', agentName: 'feat: add shared surface provenance rail', messages: Array(47).fill({role: 'user', text: ''}), linkConfidence: 0.94, importedAtISO: '2026-03-12T18:30:00Z' } as unknown as (SessionExcerpt & { linkConfidence: number }),
  { id: '2', tool: 'Codex', agentName: 'fix: harden hourly issue watchlist recovery', messages: Array(23).fill({role: 'user', text: ''}), linkConfidence: 0.71, importedAtISO: '2026-03-12T10:15:00Z' } as unknown as (SessionExcerpt & { linkConfidence: number }),
  { id: '3', tool: 'Cursor', agentName: 'update ui density uplift view models', messages: Array(15).fill({role: 'user', text: ''}), linkConfidence: 0.99, importedAtISO: '2026-03-11T14:20:00Z' } as unknown as (SessionExcerpt & { linkConfidence: number }),
  { id: '4', tool: 'Copilot', agentName: 'refactor narrative surface settings view', messages: Array(8).fill({role: 'user', text: ''}), linkConfidence: 0.35, importedAtISO: '2026-03-11T09:05:00Z' } as unknown as (SessionExcerpt & { linkConfidence: number }),
];
                ...
                {Array.from({length: 24}).map(() => {
                  ...
                  return (
                    <div 
                      ...
                      style={{
                        backgroundColor: intensity > 0.8 ? 'var(--blue-500, rgba(59,130,246,1))' : 
                                         intensity > 0.5 ? 'var(--blue-500, rgba(59,130,246,1))' : 
                                         intensity > 0.2 ? 'var(--blue-500, rgba(59,130,246,1))' : 
                                         'var(--bg-primary, rgba(255,255,255,1))',
                        opacity: intensity > 0.8 ? 1 : intensity > 0.5 ? 0.6 : intensity > 0.2 ? 0.3 : 0.05
                      }}
                    />
                  );
                })}
            ...
            <ArticleSection title="Session Quality">
              <div className="flex items-center justify-between p-4 border border-border-subtle rounded-xl bg-bg-primary">
                 <div className="flex flex-col">
                   <span className="text-xs text-text-muted">Link Confidence Average</span>
                    ...
                    <div className="flex items-center gap-3 truncate min-w-0 pr-4">
                      ...
                      <div className="flex flex-col truncate min-w-0">
                        <div className="flex items-center gap-2 truncate min-w-0">
                          <span className="text-sm font-medium text-text-primary truncate">{session.tool}</span>
                          <span className="text-sm text-text-secondary truncate">{session.agentName || 'Unknown implementation session'}</span>
                        </div>
                        <div className="flex items-center gap-3 mt-1 text-xs text-text-muted shrink-0">
                          ...
                          <span className={clsx("flex items-center gap-1 rounded-md border px-1.5 py-0.5", conf.color)}>

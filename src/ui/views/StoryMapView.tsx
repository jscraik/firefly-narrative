...
import { Network, Radar, ShieldAlert, ShieldCheck } from "lucide-react";
					...
					<header className="mb-8 flex flex-col gap-4 border-b border-border-subtle pb-6 sm:flex-row sm:items-end sm:justify-between">
						<div className="flex flex-col gap-2">
							...
							<p className="max-w-xl text-sm leading-6 text-text-secondary">
								Topology and prioritization view for pressure points, weak
								joins, and next inspection.
							</p>
						</div>
						<div>
							<button
							...
							{topologyLanes.map((lane) => (
								...
								>
									<div className="flex items-center justify-between gap-3">
										...
										<span
											className={`inline-flex rounded-full border px-2 py-0.5 text-[0.625rem] font-semibold uppercase tracking-[0.14em] ${toneClasses[lane.tone]}`}
										>
											lane
										</span>
									</div>
									<div className="mt-4 space-y-3">
										{lane.items.map((item, _index) => (
											<div
												...
												style={
													{
														"--base-accent": `var(--color-accent-${lane.tone})`,
							...
							<div className="mt-4 space-y-3">
								{viewModel.highlights.map((highlight) => (
									<button
										...
										className={clsx(
											"w-full rounded-[1.25rem] border p-4 text-left transition hover:-translate-y-0.5 hover:bg-bg-primary",

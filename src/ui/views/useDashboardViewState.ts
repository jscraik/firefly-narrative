...
export function useDashboardViewState({
	...
	const [emptyReason, setEmptyReason] = useState<DashboardEmptyReason | null>(
		null,
	);
	...
	const sequenceRef = useRef<{ lastKey: string; ts: number }>({
		lastKey: "",
		ts: 0,
	});
	...
	const failureMetadataRef = useRef<DashboardRequestFailureMetadata | null>(
		null,
	);
	const droppedRequestDiagnosticsRef = useRef<
		DashboardDroppedRequestDiagnostic[]
	>([]);
	...
	useEffect(() => {
		if (dashboardState === "default" && !hasPaintedRef.current) {
			...
			requestAnimationFrame(() => {
				...
				try {
					performance.measure(
						"dashboard_first_data_paint",
						"dashboard_route_activation",
						"dashboard_data_paint_end",
					);
					performance.measure(
						"dashboard_tti",
						"dashboard_route_activation",
					...
					const paintMeasure = performance.getEntriesByName(
						"dashboard_first_data_paint",
					)[0];
					const ttiMeasure = performance.getEntriesByName("dashboard_tti")[0];

					if (paintMeasure && paintMeasure.duration > 1200) {
						/* slow paint threshold exceeded — metrics available via performance API */
					}
		...
		if (
			...
			performance.getEntriesByName("dashboard_filter_start").length > 0
		) {
			requestAnimationFrame(() => {
				performance.mark("dashboard_filter_end");
				try {
					performance.measure(
						"dashboard_interaction_latency",
						"dashboard_filter_start",
						"dashboard_filter_end",
					);
					const measure = performance
						.getEntriesByName("dashboard_interaction_latency")
						.pop();
	...
	const activeRepoId =
		repoState.status === "ready" ? repoState.repo.repoId : null;
	...
	const recordDroppedRequest = useCallback(
		(entry: DashboardDroppedRequestDiagnostic) => {
			...
			const retainedEntries = droppedRequestDiagnosticsRef.current
				.filter(
					(diagnostic) =>
						now - Date.parse(diagnostic.droppedAtIso) <=
						DASHBOARD_DROPPED_REQUEST_TTL_MS,
				)
				.concat(entry);
	...
	const fetchStats = useCallback(
		async (isLoadMore = false) => {
			...
			const isStaleRequest = (
				reason: DashboardDroppedRequestDiagnostic["reason"],
			) => {
			...
			try {
				const data = useBrowserMockData
					? {
							...
							repo: {
								...
								name:
									repoState.repo.root.split("/").filter(Boolean).pop() ||
									MOCK_DASHBOARD_STATS.repo.name,
							},
							timeRange,
						}
				...
				if (isStaleRequest("superseded")) return;
				const failure = classifyDashboardFailure(
					cause,
					captureReliabilityStatus,
				);
				...
				if (failure.state === "permission_denied") {
					trackDashboardEvent({
						...
						payload: { repo_id: requestMeta.repoId, message: failure.message },
					});
				}
				if (!failure.canRetry) {
					trackDashboardEvent({
						event: "dashboard_retry_budget_exhausted",
						payload: { repo_id: requestMeta.repoId },
					});
				}
	...
	const visibleFiles = useMemo(() => {
		...
		return stats.topFiles.files.filter((file) =>

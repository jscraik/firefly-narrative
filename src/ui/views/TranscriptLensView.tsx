...
export function TranscriptLensView({
	...
	const transcriptMessages: TranscriptResult[] =
		repoState.status === "ready"
			...
			: [];
	const resultGroups: TranscriptResult[] =
	...
	const groupedResults = resultGroups.reduce<
		Array<{
			groupLabel: string;
			items: typeof resultGroups;
		}>
	>((groups, result) => {
		const groupLabel = result.linkedCommitSha
			? `Linked ${result.linkedCommitSha.slice(0, 7)}`
			: `Unlinked ${result.tool}`;
				...
				<div className="mx-auto grid max-w-[100rem] gap-5 xl:grid-cols-[0.8fr_1.22fr_0.72fr]">
					...
					>
						<div className="flex items-start justify-between gap-4">
							<div>
								...
								<h2 className="mt-1 text-lg font-semibold text-text-primary">
									Ask transcript-first questions
								</h2>
							</div>
							<AuthorityCue
						...
						<div className="mt-3 rounded-xl border border-border-light bg-bg-primary p-3.5">
							...
							<p className="mt-2.5 text-sm leading-6 text-text-secondary">
								The search bar and chips stay visible first so this screen reads
								like a query console, not another summary page.
							</p>
						</div>
						...
						<div className="mt-3 space-y-2.5">
							{suggestedQueries.map((highlight) => (
								<button
									...
									type="button"
									onClick={() =>
										highlight.action && onAction?.(highlight.action)
									}
					...
					<article className="rounded-2xl border border-border-subtle bg-bg-subtle p-4">
						<div className="flex items-start justify-between gap-4">
							<div>
								...
								<h2 className="mt-1 text-lg font-semibold text-text-primary">
									Quoted snippets and source joins
								</h2>
							</div>
						...
						<div className="mt-3 space-y-3">
							{groupedResults.map((group) => (
								<section
									...
									className="rounded-xl border border-border-light bg-bg-primary p-3"
								>
									<div className="flex flex-wrap items-center justify-between gap-2">
										<div>
											<Eyebrow>Result lane</Eyebrow>
											<h3 className="mt-1 text-sm font-semibold text-text-primary">
												{group.groupLabel}
											</h3>
										</div>
										<span className="rounded-full border border-border-light bg-bg-secondary px-2.5 py-1 text-[0.6875rem] font-semibold uppercase tracking-[0.14em] text-text-secondary">
											{group.items.length} hit
									...
									<div className="mt-3 space-y-2.5">
										{group.items.map((result) => (
											<article
												...
												className="group rounded-xl border border-border-light bg-bg-primary p-3.5 transition-all duration-200 ease-out active:scale-[0.98] active:duration-75 hover:border-accent-blue-light/50 hover:bg-bg-primary"
											>
												<div className="flex flex-wrap items-start justify-between gap-3">
													<div>
														<p className="text-sm font-semibold text-text-primary">
															{result.tool} · {result.role}
														</p>
														<p className="mt-1 text-xs uppercase tracking-[0.16em] text-text-muted">
															{result.importedAtISO
																? new Date(result.importedAtISO).toLocaleString(
																		"en-GB",
																		{
																			day: "2-digit",
																			month: "short",
													...
													<span className="rounded-full border border-border-light bg-bg-secondary px-2.5 py-1 text-[0.6875rem] font-semibold uppercase tracking-[0.14em] text-text-secondary transition-colors group-hover:border-accent-blue-light/30">
														{result.linkedCommitSha
															? result.linkedCommitSha.slice(0, 7)
															: "Unlinked"}
													</span>
												</div>
													...
													<button
														...
														onClick={() => {
															if (result.linkedCommitSha) {
																onAction?.({
																	type: "open_evidence",
																	evidenceId: `commit:${result.linkedCommitSha}`,
																});
																return;
															}
					...
					<article className="rounded-2xl border border-border-subtle bg-bg-subtle p-4">
						<div className="flex items-start justify-between gap-4">
							...
							<AuthorityCue
								authorityTier={
									viewModel.metrics[0]?.authorityTier ??
									viewModel.heroAuthorityTier
								}
								authorityLabel={
									viewModel.metrics[0]?.authorityLabel ??
									viewModel.heroAuthorityLabel
								}
							/>
						</div>
						...
						<div className="mt-3 rounded-xl border border-border-light bg-bg-primary p-3.5">
							...
							<p className="mt-1.5 text-sm leading-6 text-text-secondary">
								Search coverage belongs on the side rail. The quoted snippet
								stack should remain the dominant primitive so the operator can
								read, test, and pivot quickly.
							</p>
						</div>
					</article>
				</div>

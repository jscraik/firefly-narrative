...
const toneClasses: Record<SurfaceTone, { border: string; bg: string; text: string; dot: string }> = {
  blue: {
...
type SurfaceActionHandler = (action: NonNullable<SurfaceTableRow['action']>) => void;

function authorityShortLabel(tier?: SurfaceAuthorityCue['authorityTier']): string {
  switch (tier) {
    case 'live_repo':
      return 'Repo';
    case 'live_capture':
...
export function AuthorityCue({ authorityTier, authorityLabel }: SurfaceAuthorityCue) {
...
export function MetricCard({ metric }: { metric: SurfaceMetric & SurfaceAuthorityCue }) {
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between">
                ...
                {highlight.action && (
                  <ArrowUpRight
                    className={clsx('h-3.5 w-3.5 opacity-40 transition-opacity group-hover:opacity-100', tone.text)}
                  />
                )}
              </div>

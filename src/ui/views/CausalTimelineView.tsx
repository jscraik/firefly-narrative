...
export function CausalTimelineView({
	...
	const timelineNodes =
		repoState.status === "ready" ? repoState.model.timeline : [];
	const sessionExcerpts =
		repoState.status === "ready" ? (repoState.model.sessionExcerpts ?? []) : [];
	const linkedSessionCount = sessionExcerpts.filter((session) =>
		Boolean(session.linkedCommitSha),
	).length;
							...
							<div className="flex items-start justify-between gap-4">
								<div>
									...
									<h2 className="mt-1 text-lg font-semibold text-text-primary">
										Ordered commit and milestone sequence
									</h2>
								</div>
								<AuthorityCue
							...
							<div className="mt-4 space-y-0">
								{timelineNodes.length > 0 ? (
									timelineNodes.map((node, index) => {
										const linkedSessionsForNode = sessionExcerpts.filter(
											(session) => session.linkedCommitSha === node.id,
										).length;
										const stateTone =
										...
										return (
											...
											>
												{index < timelineNodes.length - 1 ? (
													<div
														className="absolute left-[0.75rem] top-7 h-[calc(100%-0.75rem)] w-px bg-border-light"
														aria-hidden="true"
													/>
												) : null}
																	...
																	className={clsx(
																		"inline-flex rounded-full border px-2 py-0.5 text-[0.625rem] font-semibold uppercase tracking-[0.14em]",
																		{
																			"border-accent-green-light bg-accent-green-bg text-accent-green":
																				stateTone === "green",
																			"border-accent-blue-light bg-accent-blue/10 text-accent-blue":
																				stateTone === "blue",
									...
									<article className="rounded-[1.25rem] border border-dashed border-border-light bg-bg-primary/70 p-4 text-sm text-text-secondary">
										Timeline evidence has not been indexed yet. Import sessions
										or open the repo to build the first causal spine.
									</article>
								)}
							</div>
						</article>
							...
							<article className="rounded-[1.6rem] border border-border-subtle bg-bg-secondary/80 p-4">
								<div className="flex items-start justify-between gap-4">
									<div>
										...
										<h2 className="mt-1 text-lg font-semibold text-text-primary">
											What makes the sequence safe to repeat
										</h2>
									</div>
							...
							<article className="rounded-[1.6rem] border border-border-subtle bg-bg-secondary/80 p-4">
								<div className="flex items-start justify-between gap-4">
									<div>
										...
										<h2 className="mt-1 text-lg font-semibold text-text-primary">
											Session joins and trust posture
										</h2>
									</div>
									<AuthorityCue
										authorityTier={
								...
								<div className="mt-3 rounded-[1.1rem] border border-border-light bg-bg-primary/70 p-3.5">
									<div className="flex items-center gap-2 text-sm font-semibold text-text-primary">
										<ShieldCheck
											className={`h-4 w-4 ${linkedSessionCount > 0 ? "text-accent-green" : "text-accent-amber"}`}
										/>

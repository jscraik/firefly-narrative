...
import {
	...
	NextLanesCard,
	PressureWatchCard,
} from "../components/dashboard/DashboardAttentionSections";
...
export function DashboardMainContent({
	...
	const repoBranch =
		repoState.status === "ready" ? repoState.repo.branch : null;
	...
	const activityData = useMemo(
		() =>
			stats.currentPeriod.trend.map((p) => ({
				date: p.date,
				value: p.commitCount,
			})),
					...
					<section className="grid gap-4 xl:grid-cols-[0.75fr_1.25fr]">
						...
						<article className="rounded-3xl border border-border-subtle bg-bg-subtle p-4 shadow-card">
							<div className="flex flex-wrap items-center gap-2">
								...
								<span
									className={`rounded-full border px-3 py-1 text-[0.6875rem] font-semibold uppercase tracking-[0.18em] ${TONE_BADGE[aiPct >= 30 ? "violet" : "blue"]}`}
								>
									{aiPct}% AI
								</span>
							</div>
							...
							<div className="mt-4 space-y-2">
								...
								<div className="flex items-center gap-2 rounded-xl border border-border-light bg-bg-primary px-3 py-2 text-sm">
									<AlertTriangle
										className={`h-3.5 w-3.5 shrink-0 ${trustIsHealthy ? "text-accent-green" : "text-accent-amber"}`}
									/>
									<span className="truncate text-text-secondary">
										{trustIsHealthy
											? "Trust is healthy — proceed to repo"
											: "Trust gate active — resolve before handoff"}
									</span>
								</div>
							</div>
						</article>
					...
					<section className="grid gap-4 xl:grid-cols-[1fr_1fr]">
						...
						{toolChartData.length > 0 ? (
							<article className="rounded-3xl border border-border-subtle bg-bg-subtle p-4">
								<div className="mb-3 flex items-center justify-between gap-3">
									<div>
										<Eyebrow>Tool distribution</Eyebrow>
										<h3 className="mt-0.5 text-sm font-semibold text-text-primary">

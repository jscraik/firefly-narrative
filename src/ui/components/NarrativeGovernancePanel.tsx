import type { NarrativeObservabilityMetrics, NarrativeRolloutReport } from '../../core/types';
...
function statusClasses(status: NarrativeRolloutReport['status']): string {
  if (status === 'rollback') return 'border-accent-red-light bg-accent-red-bg text-accent-red';
  if (status === 'watch') return 'border-accent-amber-light bg-accent-amber-bg text-accent-amber';
  return 'border-accent-green-light bg-accent-green-bg text-accent-green';
}

export function NarrativeGovernancePanel({ report, observability }: NarrativeGovernancePanelProps) {
    ...
    <div className="card p-5">
      ...
      <div className="mt-3 grid grid-cols-2 gap-2 text-xs text-text-secondary sm:grid-cols-4">
        <div className="rounded-md border border-border-subtle bg-bg-primary px-2.5 py-2">
          ...
          <div className="mt-1 text-sm font-semibold text-text-primary">{observability.layerSwitchedCount}</div>
        </div>
        <div className="rounded-md border border-border-subtle bg-bg-primary px-2.5 py-2">
          <div className="text-text-muted">Evidence opens</div>
          <div className="mt-1 text-sm font-semibold text-text-primary">{observability.evidenceOpenedCount}</div>
        </div>
        <div className="rounded-md border border-border-subtle bg-bg-primary px-2.5 py-2">
          <div className="text-text-muted">Fallback usage</div>
          <div className="mt-1 text-sm font-semibold text-text-primary">{observability.fallbackUsedCount}</div>
        </div>
        <div className="rounded-md border border-border-subtle bg-bg-primary px-2.5 py-2">
          <div className="text-text-muted">Kill-switch triggers</div>
          <div className="mt-1 text-sm font-semibold text-text-primary">{observability.killSwitchTriggeredCount}</div>
        </div>
      </div>

      <div className="mt-4 space-y-2">
        {report.rubric.map((metric) => (
          <div key={metric.id} className="rounded-md border border-border-subtle bg-bg-primary px-3 py-2">

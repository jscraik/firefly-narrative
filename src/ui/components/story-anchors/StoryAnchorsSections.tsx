...
export function HooksStatusRow(props: {
  ...
  const { hookInstalled, hooksDir, busy, canRun, onInstallHooks, onUninstallHooks, onRefresh } = props;
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      <ActionButton label="Install hooks" disabled={busy || !canRun} onClick={onInstallHooks} />
      <ActionButton label="Uninstall hooks" disabled={busy || !canRun} onClick={onUninstallHooks} />
  ...
  return (
    <div className="mt-2 flex flex-col gap-2 rounded-md border border-border-subtle bg-bg-tertiary px-3 py-2">
      ...
      <div className="flex flex-wrap gap-2">
        <ActionButton label="Refresh indexed status" disabled={busy || !canRunRepoActions} onClick={onRefreshIndexedStatus} />
        <ActionButton label="Import sessions notes" disabled={busy || !canRunRepoActions} onClick={onImportSessionsNotes} />
        <ActionButton label="Export sessions notes" disabled={busy || !canRunRepoActions} onClick={onExportSessionsNotes} />
        <ActionButton label="Migrate attribution ref" disabled={busy || !canRunRepoActions} accent onClick={onMigrateAttributionRef} />
        <ActionButton label="Reconcile (dry-run)" disabled={busy || !canRunRepoActions} onClick={onReconcileDryRun} />
...
export function CommitActionsCard(props: {
  ...
  return (
    <div className="mt-2 flex flex-col gap-2 rounded-md border border-border-subtle bg-bg-tertiary px-3 py-2">
      ...
      <div className="flex flex-wrap gap-2">
        <ActionButton label="Import sessions note" disabled={busy || !canRunCommitActions} onClick={onImportSessionNote} />
        <ActionButton label="Export sessions note" disabled={busy || !canRunCommitActions} onClick={onExportSessionNote} />
        <ActionButton label="Reconcile (no write)" disabled={busy || !canRunCommitActions} onClick={onReconcileNoWrite} />
        <ActionButton label="Reconcile (write)" disabled={busy || !canRunCommitActions} accent onClick={onReconcileWrite} />
        <ActionButton label="Migrate attribution ref" disabled={busy || !canRunCommitActions} accent onClick={onMigrateAttributionRef} />
      </div>
    </div>
  );
}

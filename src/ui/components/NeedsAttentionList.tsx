    ...
    <div className="card p-4">
      ...
      <div className="mt-3 space-y-3">
        {issues.map((issue) => (
          <div
            key={issue.id}
            className="rounded-lg border border-accent-amber-light bg-accent-amber-bg p-3"
          >
            <div className="text-xs font-semibold text-accent-amber">{issue.title}</div>
            <div className="text-[11px] text-text-secondary mt-1 whitespace-pre-wrap">{issue.message}</div>

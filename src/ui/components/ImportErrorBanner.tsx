...
function getErrorHelp(error: string): {
	...
	if (
		lowerError.includes("no readable messages found in the kimi context log")
	) {
		return {
			title: "Kimi Log Format Issue",
			description:
				"The file was read successfully, but we could not find any messages in the expected format. Kimi logs should be JSON Lines (.jsonl) with records containing role and content fields.",
			actions: [
				{
	...
	if (lowerError.includes("json") && lowerError.includes("parse")) {
		return {
			...
			description:
				"The file could not be parsed as JSON. Please ensure the file contains valid JSON with proper syntax.",
			actions: [
				{
	...
	if (lowerError.includes("file") || lowerError.includes("not found")) {
		return {
			...
			description:
				"Could not read the selected file. Please ensure the file exists and you have permission to access it.",
			actions: [],
		};
	}
		...
		<div className="rounded-xl border border-accent-error-warm-light bg-accent-error-warm-bg p-4 text-sm">
			<div className="flex items-start gap-3">
				...
				<div className="flex-1 min-w-0">
					{help ? (
						<>
							<div className="mb-1 font-semibold text-accent-red">
								{help.title}
							</div>
							<div className="mb-3 leading-relaxed text-text-secondary">
								{help.description}
							</div>

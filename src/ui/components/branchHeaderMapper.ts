...
const GENERIC_REPO_DESCRIPTION = 'Recent branch activity and code-change summary.';
...
function deriveTraceMetricReason(model: BranchViewModel): HeaderMetricUnavailableReason | null {
...
function deriveDescription(model: BranchViewModel): string {
  ...
  return model.source === 'git' ? GENERIC_REPO_DESCRIPTION : GENERIC_DEMO_DESCRIPTION;
}
...
export function deriveBranchHeaderViewModel(input: BranchHeaderDeriveInput): BranchHeaderViewModel {
  const {
    mode,
    repoStatus,
    model,
...
export function normalizeRepoKey(repoKey?: string | null): string {
  ...
  const normalized = repoKey.trim().replace(/\\/g, '/').replace(/\/+$/g, '').toLowerCase();
...
export function createBranchHeaderRequestIdentityKey(input: RequestIdentityInput): string {

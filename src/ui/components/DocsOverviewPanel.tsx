 ...
 */
export function DocsOverviewPanel({
	repoRoot,
	onClose,
}: DocsOverviewPanelProps) {
					...
					{selectedDoc ? (
						...
						>
							...
							<h3 className="mb-2 text-lg font-semibold text-text-primary">
								No Narrative docs found
							</h3>

							<p className="text-sm text-text-secondary max-w-[42ch] leading-relaxed mb-6">
								Narrative renders markdown files inside{" "}
								<span className="font-mono text-xs bg-bg-subtle px-1.5 py-0.5 rounded border border-border-light">
									.narrative/
								</span>
								. Mermaid diagrams render from fenced{" "}
								<span className="font-mono text-xs bg-bg-subtle px-1.5 py-0.5 rounded border border-border-light">
									```mermaid
								</span>{" "}
								blocks.

...
export function SessionImportPanel({ repoId }: SessionImportPanelProps) {
  ...
  return (
    <div className="bg-bg-card rounded-lg border border-border-light p-4">
      ...
      {error && (
        <div className="mb-4 p-3 bg-accent-red-bg border border-accent-red-light rounded-md flex items-start gap-2">
          ...
          <div className="flex items-center justify-between mb-2">
            ...
            {selectedPaths.size > 0 && (
              <button
                ...
                disabled={importing}
                className="flex items-center gap-2 px-3 py-1.5 text-sm font-medium text-text-inverse bg-surface-strong rounded-md hover:bg-surface-strong-hover disabled:opacity-50 transition-colors"
              >
                ...
                >
                  <div className="flex items-center gap-3">
                    <input
                      ...
                      className="w-4 h-4 text-accent-blue border-border-light rounded focus:ring-2 focus:ring-accent-blue"
                    />
                  ...
                  <button
                    ...
                    disabled={importing}
                    className="text-xs font-medium text-accent-blue hover:text-text-primary disabled:opacity-50"
                  >
                    Import
                  </button>
                </div>
          ...
          {result.succeeded.length > 0 && result.succeeded.some(s => s.warnings.length > 0) && (
            <div className="mt-2 space-y-1">
              {result.succeeded
                ...
                .map((s) => (
                  <p key={s.path} className="text-xs text-accent-amber">

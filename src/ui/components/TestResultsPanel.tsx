...
function TestCaseRow({ test, onFileClick }: { test: TestCase; onFileClick?: (path: string) => void }) {
  ...
  return (
    <div className="border-b border-accent-red-light py-3 last:border-0">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {test.errorMessage && (
            <div className="mt-1 text-xs text-accent-red font-mono">{test.errorMessage}</div>
          )}
          {filePath ? (
            <button
              type="button"
        ...
        <div id={panelId} className="border-t border-border-subtle px-5 pb-5">
          ...
          {hasFailures ? (
            ...
            <div className="mt-4 flex items-center gap-2 rounded-lg border border-accent-green-light bg-accent-green-bg px-4 py-3">
              ...
              <span className="text-sm text-text-secondary">All tests passed</span>
            </div>
          )}
        </div>
      )}

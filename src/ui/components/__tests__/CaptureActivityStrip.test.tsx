...
function makeEvent(id: number, message: string, commitShas: string[] = ['abc123']): ActivityEvent {
  return {
    id,
...
describe('CaptureActivityStrip', () => {
  ...
  it('clears stale drawer items when a refresh request fails', async () => {
    ...
    await waitFor(() => {
      expect(onRequestAll).toHaveBeenCalledTimes(2);
      expect(screen.getByText('No activity for this filter yet.')).toBeInTheDocument();
    });

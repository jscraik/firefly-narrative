...
describe('deriveBranchHeaderViewModel', () => {
  ...
  it('uses deterministic title fallback for git model', () => {
    const vm = deriveBranchHeaderViewModel({
      mode: 'repo',
      repoStatus: 'ready',
      model: createModel({ title: '', meta: { branchName: '', headSha: 'a1b2c3d4e5f6' } }),
    });
  ...
  it('marks AI metrics unavailable when trace state is inactive', () => {
    ...
    expect(vm.kind).toBe('full');
    if (vm.kind === 'full') {
      expect(vm.metrics.prompts).toEqual({ kind: 'unavailable', reason: 'NO_TRACE_DATA' });
      expect(vm.metrics.responses).toEqual({ kind: 'unavailable', reason: 'NO_TRACE_DATA' });
    }
  });
});
...
describe('request identity normalization', () => {
  it('normalizes repo keys deterministically', () => {
    ...
    expect(normalizeRepoKey('C:\\Users\\Jamie\\Repo\\')).toBe('c:/users/jamie/repo');
  });

  it('normalizes equivalent filter objects to the same key', () => {
    const keyA = normalizeFilterKey({ type: 'date-range', dateRange: { from: '2026-02-01', to: '2026-02-18' } });
    const keyB = normalizeFilterKey({ dateRange: { to: '2026-02-18', from: '2026-02-01' }, type: 'date-range' });

...
describe('StoryAnchorsPanel', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockGetRepoHooksStatus.mockResolvedValue({ installed: false, hooksDir: '/repo/.git/hooks' });
    ...
    mockImportSessionLinkNotesBatch.mockResolvedValue({ imported: 0, total: 0 });
  ...
  it('ignores stale selected-commit status completion after commit switch', async () => {
    const oldStatusDeferred = createDeferred<
      Array<{
        commitSha: string;
    ...
    mockGetStoryAnchorStatus.mockImplementation(async (_repoId: number, shas: string[]) => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText(/Notes:\s+attribution ✓ · sessions\s+✓ · lineage —/)).toBeInTheDocument();
    });
    ...
    expect(screen.getByText('22222222')).toBeInTheDocument();
    expect(screen.getByText(/Notes:\s+attribution ✓ · sessions\s+✓ · lineage —/)).toBeInTheDocument();
    expect(screen.queryByText(/Notes:\s+attribution — · sessions\s+— · lineage ✓/)).not.toBeInTheDocument();
  });

  it('ignores stale hooks-status completion from previous repo', async () => {
    const oldHooksDeferred = createDeferred<{ installed: boolean; hooksDir: string }>();
    ...
    await act(async () => {
      oldHooksDeferred.resolve({ installed: true, hooksDir: '/repo-1/.git/hooks' });
  ...
  it('does not keep action loading state after repo changes mid-action', async () => {
    ...
    mockGetRepoHooksStatus.mockImplementation(async (repoId: number) => {
      if (repoId === 1) return { installed: false, hooksDir: '/repo-1/.git/hooks' };
      return { installed: false, hooksDir: '/repo-2/.git/hooks' };
    });
    ...
    await waitFor(() => {
      expect(mockGetRepoHooksStatus).toHaveBeenCalledWith(2);
      expect(screen.getByRole('button', { name: 'Install hooks' })).not.toBeDisabled();
    });

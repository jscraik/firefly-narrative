...
describe('SessionImportPanel', () => {
  ...
  it('ignores stale batch import completion after repo switch', async () => {
    ...
    mockScanForSessionFiles.mockResolvedValue([
      { path: '/tmp/session-a.json', tool: 'codex', detectedAt: '2026-02-24T00:00:00.000Z' },
    ]);
    mockImportSessionFiles.mockImplementationOnce(async () => importDeferred.promise);
    ...
    await waitFor(() => {
      expect(mockImportSessionFiles).toHaveBeenCalledWith(1, ['/tmp/session-a.json']);
    });
    ...
    await act(async () => {
      importDeferred.resolve({
        total: 1,
        succeeded: [{ path: '/tmp/session-a.json', sessionId: 'session-a', warnings: [] }],
        failed: [],
      });
    ...
    expect(screen.queryByText('Successfully imported 1 session')).not.toBeInTheDocument();
  });

  it('ignores stale scan completion from previous repo context', async () => {
    const staleScan = createDeferred<
    ...
    await act(async () => {
      staleScan.resolve([
        { path: '/tmp/stale-session.json', tool: 'codex', detectedAt: '2026-02-24T00:00:00.000Z' },
      ]);
  ...
  it('keeps paths selected after import starts when new paths are added mid-flight', async () => {
    ...
    mockScanForSessionFiles.mockResolvedValue([
      { path: '/tmp/session-a.json', tool: 'codex', detectedAt: '2026-02-24T00:00:00.000Z' },
      { path: '/tmp/session-b.json', tool: 'codex', detectedAt: '2026-02-24T00:00:00.000Z' },
    ]);
    mockImportSessionFiles.mockImplementationOnce(async () => importDeferred.promise);
    ...
    const firstCheckbox = screen.getByLabelText('Select session-a.json') as HTMLInputElement;
    const secondCheckbox = screen.getByLabelText('Select session-b.json') as HTMLInputElement;
    expect(firstCheckbox.checked).toBe(false);
    expect(secondCheckbox.checked).toBe(true);
    expect(screen.getByText('Successfully imported 1 session')).toBeInTheDocument();
  });
});

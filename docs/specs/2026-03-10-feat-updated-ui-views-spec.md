...
Trace Narrative already exposes a broad operator-oriented mode taxonomy in `src/core/types.ts`, but the shell contract has lagged behind the taxonomy. The app currently has strong anchor experiences for `dashboard`, `repo`, and `docs`, yet the broader narrative, evidence, workspace, integrations, health, and configure lanes need a shared behavioral contract so implementation does not drift into ad hoc placeholders, inconsistent trust messaging, or clone-like information architecture.
...
- Brainstorm source defines the sectioned surface direction and non-goals.
...
- Added explicit data-authority and action-availability contracts so surface content cannot overstate source certainty.
...
2. Preserve anchor experiences:
   `dashboard` remains the aggregate narrative entry, `repo` remains the deep narrative evidence workspace, and `docs` remains the documentation and repo-doc inspection lane.
3. Define how non-anchor modes render as surface views without inventing a second routing system.
...
3. Wiring every surface mode to fully live backend data in the first implementation pass.
...
3. Which modes use anchor views versus surface views.
4. Trust behavior for shared surfaces that depend on capture reliability or repo state.
...
7. `src/ui/views/NarrativeSurfaceView.tsx` owns the shared presentation contract for non-anchor modes.
8. `src/ui/views/narrativeSurfaceData.ts` owns per-mode information architecture, copy contract, and section-level summary content until richer data adapters exist.
...
1. `Mode`
   Canonical top-level route selector. Current values include anchor modes (`dashboard`, `repo`, `docs`) and surface modes such as `live`, `sessions`, `tools`, `work-graph`, and `status`.
2. `AnchorMode`
   `dashboard | repo | docs`. Uses dedicated views with distinct behavior and deeper interaction models.
3. `SurfaceMode`
   Every `Mode` except `dashboard | repo | docs`. Renders via the shared surface contract.
4. `ViewSection`
   One of `Narrative`, `Evidence`, `Workspace`, `Integrations`, `Health`, or `Configure`. Determines sidebar grouping and page-level framing.
5. `RepoState`
   Top-level repo availability state: `idle`, `loading`, `error`, or `ready`. Shared across dashboard, repo, docs, and surface views.
6. `DashboardFilter`
...
8. `SurfaceTrustState`
   Normalized UI-facing trust overlay for shared surfaces. Values: `healthy` or `degraded`. Derived from `CaptureReliabilityStatus`, not independently authored by surface pages.
9. `NarrativeSurfaceViewModel`
   Presentation contract for a surface page: `section`, `title`, `subtitle`, `hero`, `metrics`, `highlights`, `activity`, `table`, `footerNote`, and `trustState`.
10. `OperatorAction`
...
1. Every `Mode` must deterministically resolve to exactly one view family: anchor view or surface view.
2. Every surface page must have exactly one `ViewSection`.
...
5. A surface page may be partially populated, but it may not present invented live certainty when its data is static, missing, or degraded.
...
10. Actions shown in shared surfaces must declare availability from current `repoState`, runtime support, and trust context; a disabled action must explain why it is blocked.
11. Phase one requires a visible authority cue for every rendered surface element:
...
12. One shared trust-normalization helper must own `CaptureReliabilityStatus -> SurfaceTrustState` conversion for both dashboard and shared surfaces; dashboard and surface pages must not fork trust conversion logic.
...
| --- | --- | --- | --- | --- |
| `dashboard` | anchor | Narrative | Narrative Brief | `derived_summary` |
| `work-graph` | surface | Narrative | Story Map | `derived_summary` |
| `assistant` | surface | Narrative | Codex Copilot | `static_scaffold` |
| `live` | surface | Evidence | Live Capture | `live_capture` |
| `sessions` | surface | Evidence | Sessions | `derived_summary` |
| `transcripts` | surface | Evidence | Transcript Lens | `static_scaffold` |
| `tools` | surface | Evidence | Tool Pulse | `static_scaffold` |
| `costs` | surface | Evidence | Cost Watch | `static_scaffold` |
| `timeline` | surface | Evidence | Causal Timeline | `derived_summary` |
| `repo` | anchor | Workspace | Repo Evidence | `live_repo` |
| `repo-pulse` | surface | Workspace | Workspace Pulse | `derived_summary` |
| `diffs` | surface | Workspace | Diff Review | `derived_summary` |
| `snapshots` | surface | Workspace | Checkpoints | `derived_summary` |
| `worktrees` | surface | Workspace | Worktrees | `static_scaffold` |
| `attribution` | surface | Workspace | Attribution Lens | `derived_summary` |
| `skills` | surface | Integrations | Codex Skills | `static_scaffold` |
| `agents` | surface | Integrations | Agent Roles | `static_scaffold` |
| `memory` | surface | Integrations | Memory Graph | `static_scaffold` |
| `hooks` | surface | Integrations | Hooks | `static_scaffold` |
| `setup` | surface | Integrations | Setup | `static_scaffold` |
| `ports` | surface | Integrations | Ports | `static_scaffold` |
| `hygiene` | surface | Health | Hygiene | `static_scaffold` |
| `deps` | surface | Health | Dependency Watch | `static_scaffold` |
| `env` | surface | Health | Env Hygiene | `static_scaffold` |
| `status` | surface | Health | Trust Center | `live_capture` |
| `docs` | anchor | Configure | Docs | `live_repo` |
| `settings` | surface | Configure | Settings | `static_scaffold` |
...
3. Rendering contract resolves:
   ...
   - every other mode renders `NarrativeSurfaceView`
...
2. Surface pages may reuse stable summary data initially, but they must frame it as operator guidance rather than canonical source-of-truth history.
3. Surface pages must provide at least one clear bridge back to the repo evidence lane where deeper inspection is required.
...
2. `dashboard` and shared surfaces consume normalized trust state.
...
1. `idle`
   Dashboard and surface may still render shell-level framing, but repo-dependent actions must remain cautious.
2. `loading`
   Repo mode renders indexing or progress UI, while surface pages continue to show shell context without claiming repo-ready facts.
3. `error`
   Repo mode renders repo-load failure state, while surface pages may continue to render bounded shell content but must soften or omit repo-path-derived detail.
4. `ready`
...
3. Dashboard chord interactions remain bounded by `DASHBOARD_CHORD_TIMEOUT_MS = 750`; surface views must not introduce longer hidden chord windows that make navigation feel inconsistent.
4. Dashboard dropped-request diagnostics remain bounded by the existing cap and TTL (`50` retained items over `24h`); surface views may add diagnostics, but they must follow bounded retention rather than unbounded in-memory growth.
...
6. Capture-reliability refresh remains deduplicated when a request is already in flight; surface pages consume the shared result instead of launching parallel refresh storms.
...
8. Leaving a surface page must not leave behind page-owned polling loops, retry timers, or focus traps.
...
2. `src/App.tsx`
   Owns shell routing, anchor or surface resolution, and action wiring.
...
11. `src/ui/views/NarrativeSurfaceView.tsx`
    Owns shared surface presentation.
12. `src/ui/views/narrativeSurfaceData.ts`
    Owns per-mode surface content contract.
13. `src/ui/components/dashboard/DashboardTrustBadge.tsx`
    Owns the current degraded-trust badge primitive reused by dashboard and shared surfaces.
14. `src/ui/views/__tests__/NarrativeSurfaceView.test.tsx`
    Owns baseline render and degraded-trust regression coverage for shared surfaces.
...
1. Per-mode live data adapters are not yet defined for every surface section.
2. Final operator telemetry schema for surface-mode interactions is not yet standardized.
3. The current surface implementation does not yet expose per-element authority metadata, so this spec defines the contract before implementation catches up.
...
10. Surface actions must stay recommendation-first: no surface page may introduce a destructive auto-remediation path without a separate explicit spec.
...
8. `action_blocked_by_state`
   A surface CTA cannot run because repo state, trust, or runtime preconditions are not met. Recovery: render the action as disabled with the blocking reason and the nearest safe alternative.

### Recovery rules

1. Shell-level navigation remains available even when repo-specific content is unavailable.
2. Trust recovery follows existing capture and ingest recovery mechanisms; surface views do not invent their own retry state machine.
...
6. Retry behavior for surface-adjacent data access follows the existing class-based contract:
   ...
   - surface pages must not introduce their own unbounded polling or silent retry loops
...
2. Budget profiles are environment-specific and inherited from the established dashboard contract:
   ...
   - `prod`
     ...
     - `offline_source`: `maxAttempts=1`, `maxTotalRetryMs=0`
     - `authority_denied`: `maxAttempts=1`, `maxTotalRetryMs=0`
3. Surface pages may reuse these budgets only through shared shell or hook helpers; they must not fork the numbers locally.
...
1. Count of rendered surface modes versus defined surface modes.
...
5. Count of surface renders by `DataAuthorityTier` mix so scaffold-heavy pages are visible during rollout.
6. Count of blocked surface actions by reason.
7. Count of stale async results discarded instead of applied.
8. Count of docs autoload attempts by outcome.

### Telemetry envelope

1. Shell and surface telemetry events must carry a stable envelope when the data is available:
   - `session_id`
   - `request_key_hash`
   - `attempt`
   - `mode`
...
| --- | --- | --- |
| Mode routing | Every `Mode` resolves to anchor or surface view deterministically | Unit test over mode mapping plus render smoke tests |
...
| Trust overlay | Degraded capture remains a visible overlay on dashboard and shared surfaces | DashboardView and NarrativeSurfaceView trust-state tests |
| Source authority | Scaffold or derived surface content is labeled and framed so it cannot be mistaken for live state | Snapshot or content assertion over representative surface views |
| Action availability | Surface CTAs explain blocked states instead of silently no-oping | Component tests for disabled or alternative CTA states |
...
1. Which surface sections should receive real live data first after the shared shell lands?
2. Should sidebar badge counts remain static placeholders initially or become live-derived before broader rollout?
3. Should top-nav continue to show only anchor experiences, or should it eventually expose section context for shared surface modes?
...
2. The contract clearly distinguishes anchor views from surface views.

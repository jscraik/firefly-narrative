...
function scoreTemporalOverlap(
  ...
  const durationMin = Math.min(session.durationMin ?? 30, MAX_SESSION_DURATION_MIN);
...
): { commitSha: string; confidence: number; temporalScore: number; fileScore: number } | null {
  const temporalScore = scoreTemporalOverlap(session, commit);
  const fileScore = scoreFileOverlap(session, commit);
  const confidence = (TEMPORAL_WEIGHT * temporalScore) + (FILE_OVERLAP_WEIGHT * fileScore);
...
function linkSessionToCommits(
  ...
  let bestResult: { commitSha: string; confidence: number; temporalScore: number; fileScore: number; timestampDistance: number } | null = null;

  for (const commit of candidates) {
    const result = calculateLinkConfidence(session, commit, threshold);
    if (result) {
      const commitTime = new Date(commit.authored_at);
      const timestampDistance = Math.abs(commitTime.getTime() - sessionEnd.getTime());
      ...
      if (!bestResult) {
        ...
        if (confidenceDiff > TIE_BREAK_MARGIN) {
          bestResult = { ...result, timestampDistance };
        } else if (Math.abs(confidenceDiff) <= TIE_BREAK_MARGIN && timestampDistance < bestResult.timestampDistance) {
          bestResult = { ...result, timestampDistance };
        }
      }
    }
  ...
  return bestResult ? { commitSha: bestResult.commitSha, confidence: bestResult.confidence, temporalScore: bestResult.temporalScore, fileScore: bestResult.fileScore } : null;
}
...
async function runCalibrationOnRepo(
  ...
  const accuracy = results.length > 0 ? (correctLinks / results.length) * 100 : 0;
...
async function runCalibration(): Promise<OverallCalibrationResults> {
  ...
  for (const repoId of repos) {
    ...
    console.log(`  Accuracy: ${result.accuracy.toFixed(1)}% (${result.truePositives}/${result.results.length})`);
  }

  // Calculate overall metrics
  const totalSessions = repoResults.reduce((sum, r) => sum + r.results.length, 0);
  const correctLinks = repoResults.reduce((sum, r) => sum + r.truePositives, 0);
  const overallAccuracy = totalSessions > 0 ? (correctLinks / totalSessions) * 100 : 0;
  const truePositives = correctLinks;
  const falsePositives = repoResults.reduce((sum, r) => sum + r.falsePositives, 0);
  const falseNegatives = repoResults.reduce((sum, r) => sum + r.falseNegatives, 0);
  ...
  if (overallAccuracy < 65) {
    recommendation = '⚠️  Accuracy below 65%. Consider further adjustments to algorithm or threshold.';
  } else if (overallAccuracy > 80) {
    recommendation = '✅ Accuracy above 80%. Excellent! Threshold validated at 0.65.';
  } else {
    recommendation = '✅ Accuracy within 65-80% range. Threshold validated at 0.65.';
  }
  ...
  console.log(`   Overall Accuracy: ${results.summary.overallAccuracy.toFixed(1)}%\n`);
...
async function main() {
  ...
  if (results.summary.overallAccuracy < 65) {
    ...
    console.log(`✅ Calibration study passed. Threshold validated at ${CONFIDENCE_THRESHOLD}.`);
    process.exit(0);
  }
}
